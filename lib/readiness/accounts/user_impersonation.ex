defmodule Readiness.Accounts.UserImpersonation do
  use Ash.Resource,
    otp_app: :Readiness,
  ...
  actions do
    ...
    action :stop_impersonation, :map do
      ...
      run Readiness.Accounts.UserImpersonation.Actions.StopImpersonation
    end
  end
  ...
  relationships do
    belongs_to :impersonator, Readiness.Accounts.User do
      source_attribute :impersonator_user_id
      description "The user who is performing the impersonation."
    end

    belongs_to :impersonated, Readiness.Accounts.User do
